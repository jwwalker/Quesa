 ...
 *
 * $Id: shadow.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
...
static void
LightCacheHit(HitList *hitlist, ShadowCache *cache);
...
int
Shadowed(
        Color           *result, /* resultant intensity */
        ...
        int             noshadow)/* If TRUE, no shadow ray is cast. */	
{
	...
	if (cp->obj) {
		...
		if (cp->obj->animtrans) {
			...
			if ((*cp->obj->methods->intersect.aggregate)(cp->obj->obj,
				&tmpray, &hitlist, SHADOW_EPSILON, &s)) {
				CacheHits++;
				return TRUE;
			}
		} else if ((*cp->obj->methods->intersect.normal)(cp->obj->obj,
...
void
ShadowStats(unsigned long *shadowrays,unsigned long * shadowhit,unsigned long* cachehit,unsigned long* cachemiss)
{
