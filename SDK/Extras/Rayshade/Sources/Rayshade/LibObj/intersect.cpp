 ...
 *
 * $Id: intersect.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
...
static void AddToHitList(HitList *hitlist, Ray *ray, Float mind,Float dist, Geom *obj);
/*
    ...
    Float       *maxdist)
{
	...
	if (IsAggregate(obj)) {
		 ...
		 * Aggregate
		 */
		if (!(*obj->methods->intersect.aggregate)
		 ...
		 * Primitive
		 */
		if (!(*obj->methods->intersect.normal)
...
static void
AddToHitList(HitList *hitlist, Ray *ray, Float mind,Float dist, Geom *obj)
{
	HitNode *np;
	Trans *list;
 ...
 */
void
IntersectStats(unsigned long *bvtests)
{
	*bvtests = BVTests;
}
