 ...
 *
 * $Id: hf.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
...
static void integrate_grid(Hf *hf,int i);
static void QueueTri(Hf *hf, hfTri *tri);
static hfTri *GetQueuedTri(Hf *hf,Float x,Float y,int flag);
static int DDA2D(Hf *hf, Vector *pos,Vector* ray,int level,Trav2D* trav,Float* maxdist);
static hfTri *CreateHfTriangle(Hf *hf, int x1,int  y1,int  x2,int y2,int x3,int y3,int which);
static int CheckCell(int x,int y,Hf* hf,Vector* ray,Vector* pos,Float* maxdist);
static Float intHftri(Vector *ray,Vector * pos, hfTri *tri);
static float minalt(int i,int j,float **hfdata);
static float maxalt(int i,int j,float **hfdata);


unsigned long HFTests, HFHits;

Hf *
HfCreate(char *filename)
{
	Hf *hf;
	FILE *fp;
	 ...
	 */
	hf->iBestSize = 1.0f / (float)hf->BestSize;
	/*
	 * Get HF size.
	 */
	...
	for (i = 0; i < hf->size; i++) {
		 ...
		 */
		if ((int)fread((char *)hf->data[i],sizeof(float),hf->size,fp)
		    != hf->size) {
	...
	for (i = 0; i < hf->lsize[0]; i++) {
		...
		for (j = 0; j < hf->lsize[0]; j++) {
			*maxptr++ = maxalt(i, j, hf->data) + FLT_EPSILON;
			*minptr++ = (float)minalt(i, j, hf->data) - FLT_EPSILON;
		}
	}
 ...
 */
static int
HfIntersect(GeomRef gref,Ray* ray, Float mindist,Float* maxdist)
{
	Hf *hf = (Hf*)gref;
 ...
 */
static int
DDA2D(Hf *hf, Vector *pos,Vector* ray,int level,Trav2D* trav,Float* maxdist)
{
	int x, y, size, posZ;
	...
	x = (int)(trav->cp.x * hf->spacing[level]);
	if (x == size)
		x--;
	y = (int)(trav->cp.y * hf->spacing[level]);
	if (y == size)
		y--;
 ...
 */
static int
CheckCell(int x,int y,Hf* hf,Vector* ray,Vector* pos,Float* maxdist)
{
	hfTri *tri1, *tri2;
	Float d1, d2;
	...
	tri1 = CreateHfTriangle(hf, x, y, x+1, y, x, y+1, TRI1);
	if (tri1)
...
static hfTri *
CreateHfTriangle(Hf *hf, int x1,int  y1,int  x2,int y2,int x3,int y3,int which)
{
	hfTri *tri;
	Float xid, yid;
	Vector tmp1, tmp2;
 ...
 */
static Float
intHftri(Vector *ray,Vector * pos, hfTri *tri)
{
...
/*ARGSUSED*/
static int
HfNormal(GeomRef gref,Vector* /*pos*/,Vector* nrm,Vector* gnrm)
{
	Hf *hf = (Hf*)gref;
...
/*ARGSUSED*/
static void
HfUV(GeomRef gref, Vector *pos,Vector * /*norm*/, Vec2d *uv,Vector *dpdu,Vector *dpdv)
{
	Hf *hf = (Hf*)gref;
 ...
 */
static void
HfBounds(GeomRef gref,Float bounds[2][3])
{
	Hf *hf = (Hf*)gref;
	/*
 ...
 */
static void
integrate_grid(Hf *hf, int level)
{
	int i, j, k, l, ii, ji;
	...
	for (i = 0; i < insize; i++) {
		...
		for (j = 0; j < insize; j++) {
			...
			maxinto[i][j] = max_alt + FLT_EPSILON;
			mininto[i][j] = min_alt - FLT_EPSILON;
			ji += fact;
		}
		ii += fact;
	}
 ...
 */
static hfTri *
GetQueuedTri(Hf *hf,Float x,Float y,int flag)
{
	register int i;
	register hfTri **tmp;
...
static void
HfStats(unsigned long *tests,unsigned long * hits)
{
	*tests = HFTests;
	*hits = HFHits;
}
...
HfMethods()
{
	if (iHfMethods == (Methods *)NULL) {
		...
		iHfMethods->intersect.normal = HfIntersect;
...
static void
HfMethodRegister(UserMethodType meth)
{
	if (iHfMethods)
