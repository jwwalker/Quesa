 ...
 *
 * $Id: triangle.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
...
static void	TriangleSetdPdUV( Vector p[3], Vec2d t[3], Vector *dpdu, Vector *dpdv);
static int TriangleIntersect(GeomRef gref, Ray *ray, Float mindist,Float* maxdist);
static int TriangleNormal(GeomRef gref, Vector *pos,Vector* nrm,Vector* gnrm);
static void TriangleUV(GeomRef gref, Vector *pos,Vector* norm,Vec2d* uv,Vector* dpdu,Vector* dpdv);
static void TriangleBounds(GeomRef gref, Float bounds[2][3]);
static void TriangleStats(unsigned long *tests,unsigned long* hits);
    ...
    int         flipflag)
{
	 ...
	 */
	triangle = (Triangle *)share_calloc(sizeof(Triangle),1);
...
static void
TriangleDeleteObj(GeomRef   inObj)
{
    Triangle*   theTriangle = (Triangle*)inObj;
    
    if (theTriangle->vnorm)
        Free(theTriangle->vnorm);
    if (theTriangle->uv)
        Free(theTriangle->uv);
    if (theTriangle->dpdu)
        Free(theTriangle->dpdu);
    if (theTriangle->dpdv)
        Free(theTriangle->dpdv);
    Free(theTriangle);
}
...
TriangleMethods()
{
	if (iTriangleMethods == (Methods *)NULL) {
		...
		iTriangleMethods->create = (TGeomMethod_Create)TriangleCreate;
		iTriangleMethods->methods = TriangleMethods;
		iTriangleMethods->name = TriangleName;
		iTriangleMethods->intersect.normal = TriangleIntersect;
		...
		iTriangleMethods->deleteobj = TriangleDeleteObj;
	}
 ...
 */
static int
TriangleIntersect(GeomRef gref, Ray *ray, Float mindist,Float* maxdist)
{
	Triangle *tri = (Triangle*)gref;
...
int
TriangleNormal(GeomRef gref, Vector * /*pos*/,Vector* nrm,Vector* gnrm)
{
	Triangle *tri =(Triangle*)gref;
	*gnrm = tri->nrm;
...
/*ARGSUSED*/
static void
TriangleUV(GeomRef gref, Vector *pos,Vector* /*norm*/,Vec2d* uv,Vector* dpdu,Vector* dpdv)
{
	Triangle *tri = (Triangle*)gref;
	Float d;
...
static void
TriangleBounds(GeomRef gref, Float bounds[2][3])
{
	Triangle *tri = (Triangle*)gref;
...
static void
TriangleStats(unsigned long *tests,unsigned long* hits)
{
	*tests = TriTests;
	*hits = TriHits;
}
...
TriangleSetdPdUV(
            ...
            Vector          *dpdu,  /* u and v axes (return values) */
            Vector          *dpdv)	
{
	Float scale;
	int hi, mid, lo;
...
static void
TriangleMethodRegister(UserMethodType meth)
{
	if (iTriangleMethods)
