 ...
 *
 * $Id: torus.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
 ...
 */
static int
TorusIntersect(GeomRef gref, Ray *inray,Float mindist,Float* maxdist)
{
	Torus *torus = (Torus*)gref;
	Vector pos,ray;
 ...
 */
static int
TorusNormal(GeomRef gref,Vector *rawpos,Vector* nrm,Vector* gnrm)
{
	Torus *torus = (Torus*)gref;
	Vector pos;
...
static void
TorusUV(GeomRef gref, Vector *pos,Vector* norm,Vec2d* uv,Vector* dpdu,Vector* dpdv)
{
	Torus *torus = (Torus*)gref;
	Vector npos;
 ...
 */
static void
TorusBounds(GeomRef gref, Float bounds[2][3])
{
	Torus *torus = (Torus*)gref;
...
static void
TorusStats(unsigned long *tests, unsigned long *hits)
{
	*tests = TorusTests;
	*hits = TorusHits;
}
...
TorusMethods()
{
	if (iTorusMethods == NULL) {
		...
		iTorusMethods->create = (TGeomMethod_Create)TorusCreate;
		iTorusMethods->methods  = TorusMethods;
		iTorusMethods->name = TorusName;
		iTorusMethods->intersect.normal = TorusIntersect;
...
static void
TorusMethodRegister(UserMethodType meth)
{
	if (iTorusMethods)
