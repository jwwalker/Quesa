 ...
 *
 * $Id: cone.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
 ...
 */
static int
ConeIntersect(GeomRef gref,Ray* ray,Float mindist,Float* maxdist)
{
	Cone* cone = (Cone*)gref;
 ...
 */
static int
ConeNormal(GeomRef gref,Vector* pos,Vector* nrm,Vector* gnrm)
{
	Cone* cone = (Cone*)gref;
	Vector npos;
...
static void
ConeUV(GeomRef gref, Vector *pos,Vector * /*norm*/, Vec2d *uv,Vector *dpdu,Vector *dpdv)
{
	Cone* cone = (Cone*)gref;
 ...
 */
static void
ConeBounds(GeomRef gref,Float bounds[2][3])
{
	Cone* cone = (Cone*)gref;
...
static void
ConeStats(unsigned long *tests,unsigned long * hits)
{
	*tests = ConeTests;
	*hits = ConeHits;
}
...
ConeMethods()
{
	if (iConeMethods == (Methods *)NULL) {
		...
		iConeMethods->create = (TGeomMethod_Create)ConeCreate;
		iConeMethods->methods = ConeMethods;
		iConeMethods->intersect.normal = ConeIntersect;
...
static void
ConeMethodRegister(UserMethodType meth)
{
	if (iConeMethods)
