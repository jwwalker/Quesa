 ...
 *
 * $Id: grid.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
...
static void	engrid(Geom *obj,Grid *grid);

static int  CheckVoxel();

static int pos2grid(Grid *grid, Float pos[3], int index[3]);

static void GridFreeVoxels(Grid *grid);

static int CheckVoxel(GeomList *list,Ray *ray,Float *raybounds[2][3],HitList *hitlist,unsigned long counter,Float mindist,Float*maxdist);

static int GridIntersect(GeomRef gref, Ray *ray, HitList *hitlist, Float mindist, Float *maxdist);

static int GridConvert(GeomRef gref, Geom *objlist);

static void GridBounds(GeomRef gref, Float bounds[2][3]);

Grid *
GridCreate(int x,int y,int z)
{
	Grid *grid;
...
static void
GridDeleteObj(GeomRef   inGeomRef)
{
    Grid*   theGrid = (Grid*)inGeomRef;
    
    GeomDeleteEvery(theGrid->objects);
    GeomDeleteEvery(theGrid->unbounded);
    if (theGrid->cells)
        GridFreeVoxels(theGrid);
 ...
 */
static int
CheckVoxel(GeomList *list,Ray *ray,Float *raybounds[2][3],HitList *hitlist,unsigned long counter,Float mindist,Float*maxdist)
{
	Geom *obj;
	int hit;
...
static int
GridConvert(GeomRef gref, Geom *objlist)
{
	Grid *grid = (Grid*)gref;
	int num;
...
static void
GridBounds(GeomRef gref, Float bounds[2][3])
{
	Grid *grid = (Grid*)gref;
	Geom *ltmp;
	int x, y;
...
GridMethods()
{
	if (iGridMethods == (Methods *)NULL) {
		...
		iGridMethods->create 				= (TGeomMethod_Create)GridCreate;
		iGridMethods->intersect.aggregate 	= GridIntersect;
		...
		iGridMethods->deleteobj 			= GridDeleteObj;
	}
	return iGridMethods;
}
...
static void
GridMethodRegister(UserMethodType meth)
{
	if (iGridMethods)
