 ...
 *
 * $Id: blob.cpp,v 1.1 2002-12-18 18:36:42 pepe Exp $
 *
...
static int
BlobIntersect(GeomRef gref, Ray *ray, Float mindist,Float *maxdist);
static int
BlobNormal(GeomRef gref, Vector *pos,Vector* nrm,Vector* gnrm);
static void
BlobStats(unsigned long *tests,unsigned long *hits);
static void
BlobBounds(GeomRef gref,Float bounds[2][3]);
 ...
 */
Blob *
BlobCreate(Float T, MetaList *mlist, int npoints)
{
	Blob *blob;
	int i;
	MetaList *cur;
 ...
 */
static int
MetaCompare(const void* A, const void* B)
{
	MetaInt **AA,**BB;
...
BlobMethods()
{
	if (iBlobMethods == (Methods *)NULL) {
		...
		iBlobMethods->create = (TGeomMethod_Create)BlobCreate;
		iBlobMethods->methods = BlobMethods;
		iBlobMethods->name = BlobName;
		iBlobMethods->intersect.normal = BlobIntersect;
...
static void
BlobMethodRegister(UserMethodType meth)
{
	if (iBlobMethods)
