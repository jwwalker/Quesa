 ...
 *
 * $Id: surface.cpp,v 1.1 2002-12-18 18:36:43 pepe Exp $
 *
...
/* 
 * Delete the surface object,but not it's children 
 */
void
SurfaceDeleteOne(Surface    *surf)
{
   if (surf->name)
        Free(surf->name);  
   Free(surf);
...
/* 
 * Delete the surface object and all of it's children 
 */
void
SurfaceDeleteEvery(Surface  *surf)
{
    Surface *theSurface = surf;
    while(theSurface)
    {
        Surface* nextSurface = theSurface->next;
        theSurface->next = NULL;
        SurfaceDeleteOne(theSurface);
        theSurface = nextSurface;
    }
}
...
SurfList *
SurfPop(SurfList *list)
{
	SurfList *stmp = list->next;

	share_free((voidstar)list);
	return stmp;
}

SurfList *
SurfPush(Surface *surf, SurfList *list)
