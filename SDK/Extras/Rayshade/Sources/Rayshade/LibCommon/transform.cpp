 ...
 *
 * $Id: transform.cpp,v 1.1 2002-12-18 18:36:41 pepe Exp $
 *
...
{
	if (trans->tr)
		share_free((voidstar)trans->tr);
	share_free((voidstar)trans);
}
 ...
 */
void
CoordSysTransform(Vector *origin,Vector* up,Float r,Float len,Trans* trans)
{
	RSMatrix tmp;
	Vector atmp;
...
void
TransCopy(Trans *from,Trans* into)
{
...
void
TransInvert(Trans *from,Trans* into)
{
	RSMatrix ttmp;
	/*
	 * In case into == from...
 ...
 */
void
MatrixCopy(RSMatrix *from,RSMatrix* into)
{
 ...
 */
void
VecTransform(Vector *vec, RSMatrix *trans)
{
	Vector tmp;
 ...
 */
void
NormalTransform(Vector *norm, RSMatrix *it)
{
	Vector onorm;
 ...
 */
Float
RayTransform(Ray *ray, RSMatrix *trans)
{
