...
# Last Updated: $Date: 2001-03-18 13:25:55 $
#
...
if (defined $config{"copyrightOwner"}) {
    ...
    print "No copyright owner defined in configuration file.  Using default.\n";
    HeaderDoc::APIOwner->copyrightOwner("2000 Apple Computer, Inc.");
}

if (defined $config{"defaultFrameName"}) {
    HeaderDoc::APIOwner->defaultFrameName($config{"defaultFrameName"});
} else {
    print "No default frame name defined in configuration file.  Using default.\n";
    HeaderDoc::APIOwner->defaultFrameName("index.html");
}

if (defined $config{"compositePageName"}) {
    HeaderDoc::APIOwner->compositePageName($config{"compositePageName"});
} else {
    print "No composite page name defined in configuration file.  Using default.\n";
    HeaderDoc::APIOwner->compositePageName("CompositePage.html");
}
		       ...
		       SWITCH: { # determine which type of comment we're in
				...
				if ($inFunction) {
					...
					if ($inputLines[$inputCounter] =~ /^#define/) { # if function-like macro
						...
						$declaration =~ s/\s*[{?][^;]*//; 	# handle inline functions [#2423551] A.S.D.
...
print "...done\n";
# dair, don't exit - we need to return to allow DoQuesaDocs.pl to continue
# exit 0;
